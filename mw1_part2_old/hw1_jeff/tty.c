...
Queue inQueue, outQuene, echoQuene; 
...
void ttyinit(int dev)
{
  ...
  init_queue(&outQuene, MAXBUF); 
  init_queue(&echoQuene, MAXBUF);
...
int ttywrite(int dev, char *buf, int nchar)
{
  ...
  tty = (struct tty *)devtab[dev].dvdata;   /* software data for line */
   ...
   i = 0; // set index 
     ...
     while(i<nchar) {
   if (enqueue(&outQuene, buf[i]) != FULLQUE) {
...
void irqinthandc(int dev){  
 ...
 pic_end_int();  	/*notify pic that its part is done */ 
  ...
  if (lsr & UART_LSR_DR) {		/* handling recieve interrupt */
    ...
    enqueue(&inQueue, ch);
    if (tty->echoflag)	enqueue(&echoQuene, ch);
     //if (tty->echoflag)             // if echoing wanted 
  } 
  if (lsr & UART_LSR_THRE) {	/* handling transmit interrupt */
    if (queuecount(&echoQuene))	outpt(baseport+UART_TX, dequeue(&echoQuene));
  
    if (queuecount(&outQuene)) {
     ch = dequeue(&outQuene);
