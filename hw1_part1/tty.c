...
*/
/*
* only thing left to do is the debug part output 
  ...
  // save eflag before interrupts
   ...
   for(; i < nchar; i++){
     ...
     if ((ch = dequeue(&inQueue)) != EMPTYQUE) {
      buf[i] = ch;
      // copy from ibuf to user buf  if noy emptyque
     // sprintf(log, ">%c", buf[i]);
     // debug_log(log);
    }
...
int ttywrite(int dev, char *buf, int nchar)
{
  ...
  // debug_log(log);
   if (enqueue(&outQueue, buf[i]) != FULLQUE) {
      ...
      // enable register  // reciver data | modem interrrup
    }
    //set_eflags(saved_eflags);
    sti();  // enable ints in CPU 
  }
  return nchar;
...
void irqinthandc(int dev){  
 ...
 pic_end_int();  	/*notify pic that its part is done */ 
  ...
  if (lsr & UART_LSR_DR) {		// handling recieve interrupt 
    mychars = inpt(baseport+UART_RX);	// read char, ask the device 

    // add char ch to the specified queue--returns FULLQUE if q full 
    enqueue(&inQueue, mychars);
    // adding char into q
    
    if (tty->echoflag)	enqueue(&echoQueue, mychars);
    //if (tty->echoflag)             // if echoing wanted 
  } 
  
  if (lsr & UART_LSR_THRE) {	// handling transmit interrupt 
    if (queuecount(&echoQueue))	
    // check how many char are in q passing echo of q / abc
    ...
    if (queuecount(&outQueue)) {
		// if adress has something print values 
     mychars = dequeue(&outQueue);
     outpt(baseport + UART_TX, mychars);
	 //transmit buffer out with my values mychars
    }  
  }
  outpt(baseport+UART_IER, UART_IER_RDI);
  // enbale register transmit buffer
}   
/////
