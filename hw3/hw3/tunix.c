/* file: tunix.c covim tunirernel code */
...
extern void ustart1(void), finale(void);
...
void k_init(){
  ...
  init_process_table(); /* Initialized the process table, sets curproc to 1 */
  
  while (number_of_zombie < NPROC-1) {
	sti();
	cli();
	schedule();
	}
  ...
  for(int i=1; i<NPROC;i++) {
	kprintf("\nPROCESS %d's EXIT CODE: %d\n",proctab[i].p_exitval,i);
	}
  shutdown();
